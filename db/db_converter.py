    ...
    async def convert_new_type(self, db: Optional[DB] = None):
        ...
        async def _convert_task(group_with_underline: bool):
            async with self.db.transaction() as sql_session:
                await exec_shell(
                    "ALTER TABLE `task` RENAME TO `taskold`", sql_session=sql_session
                )
                ...
                if group_with_underline:
                    await exec_shell(
                        "INSERT INTO `task` SELECT `id`,`tplid`,`userid`,`disabled`,`init_env`,`env`,`session`,`retry_count`,`retry_interval`,`last_success`,`last_failed`,`success_count`,`failed_count`,`last_failed_count`,`next`,`note`,`ctime`,`mtime`,`ontimeflg`,`ontime`,`_groups`,`pushsw`,`newontime` FROM `taskold` ",
                        sql_session=sql_session,
                    )
                else:
                    await exec_shell(
        ...
        async def _convert_tpl(group_with_underline: bool):
            async with self.db.transaction() as sql_session:
                await exec_shell(
                    "ALTER TABLE `tpl` RENAME TO `tplold`", sql_session=sql_session
                )
                if config.db_type == "sqlite3":
                    autokey = "AUTOINCREMENT"
                else:
                    autokey = "AUTO_INCREMENT"
                await exec_shell(
                    f"""CREATE TABLE IF NOT EXISTS `tpl` (
                ...
                if group_with_underline:
                    await exec_shell(
                        "INSERT INTO `tpl` SELECT `id`,`userid`,`siteurl`,`sitename`,`banner`,`disabled`,`public`,`lock`,`fork`,`har`,`tpl`,`variables`,`interval`,`note`,`success_count`,`failed_count`,`last_success`,`ctime`,`mtime`,`atime`,`tplurl`,`updateable`,`_groups` FROM `tplold` ",
                        sql_session=sql_session,
                    )
                else:
                    await exec_shell(
        ...
        async def _convert_user():
            async with self.db.transaction() as sql_session:
                await exec_shell(
                    "ALTER TABLE `user` RENAME TO `userold`", sql_session=sql_session
                )
                if config.db_type == "sqlite3":
                    autokey = "AUTOINCREMENT"
                else:
        ...
        if config.db_type == "sqlite3":
            ...
            for each in ("email", "nickname"):
                try:
                    await exec_shell(
                        f"""ALTER TABLE `{self.db.user.__tablename__}` ADD UNIQUE INDEX `ix_{self.db.user.__tablename__}_{each}` ({each})"""
                    )
        ...
        try:
            ...
            if (
                str(e).find("has no attribute '`groups`'") < 0
                and str(e).find("no such column") > -1
            ):
                logger_db_converter.debug(e)
        ...
        try:
            ...
            logger_db_converter.debug(e)
            await exec_shell(
                "ALTER TABLE `task` ADD `_groups` VARCHAR(256) NOT NULL DEFAULT 'None' "
            )
        ...
        try:
            ...
            if (
                str(e).find("has no attribute '`groups`'") < 0
                and str(e).find("no such column") < 0
            ):
                logger_db_converter.debug(e)
        ...
        try:
            ...
            logger_db_converter.debug(e)
            await exec_shell(
                "ALTER TABLE `tpl` ADD `_groups` VARCHAR(256) NOT NULL DEFAULT 'None' "
            )
        ...
        try:
            ...
            logger_db_converter.debug(e)
            if (
                str(e).find("no such column") > -1
                or str(e).find("has no attribute") > -1
            ):
                await exec_shell(
        ...
        try:
            ...
            logger_db_converter.debug(e)
            await exec_shell(
                "ALTER TABLE `user` ADD `qywx_webhook`  VARCHAR(1024) NOT NULL DEFAULT '' "
            )
        ...
        if config.db_type == "sqlite3":
            try:
                ...
                cursor = await conn.execute(
                    "SELECT id, password, userkey, password_md5 FROM user"
                )
                for row in await cursor.fetchall():
                    result = await update_shell(
                        update(User)
                        .where(User.id == row[0])
                        .values(password=row[1], userkey=row[2], password_md5=row[3])
                    )
                await cursor.close()

                cursor = await conn.execute(
                    "SELECT id, init_env, env, session FROM task"
                )
                for row in await cursor.fetchall():
                    result = await update_shell(
                        update(Task)
                        .where(Task.id == row[0])
                        .values(init_env=row[1], env=row[2], session=row[3])
                    )
                await cursor.close()
        ...
        try:
            for user in await self.db.user.list(fields=("id", "password_md5")):
                if isinstance(user["password_md5"], str) and re.match(
                    ...
                    password = (
                        await self.db.user.get(user["id"], fields=("password",))
                    )["password"]
                    await self.db.user.mod(
                        user["id"],
                        password_md5=crypto.password_hash(
                            user["password_md5"],
                            await self.db.user.decrypt(user["id"], password),
                        ),
                    )
        except Exception as e:
            logger_db_converter.debug(e)
        ...
        try:
            ...
            for user in await self.db.user.list(fields=("id", "notepad")):
                await self.db.notepad.add(
                    dict(userid=user["id"], notepadid=1, content=user["notepad"])
                )
            await _convert_user()
        except Exception as e:
            if (
                str(e).find("has no attribute 'notepad'") < 0
                and str(e).find("no such column") < 0
            ):
                logger_db_converter.debug(e)
                raise e
        ...
        try:
            ...
            async with self.db.transaction() as sql_session:
                tpls = await self.db.tpl.list(
                    fields=("id", "userid", "tpl", "variables", "init_env"),
                    sql_session=sql_session,
                )
                for tpl in tpls:
                    if not tpl["init_env"]:
                        ...
                        init_env = {}
                        if variables:
                            _tpl = await self.db.user.decrypt(
                                0 if not tpl["userid"] else tpl["userid"],
                                tpl["tpl"],
                                sql_session=sql_session,
                            )
                            try:
                                ...
                                for x in ast.find_all(Filter):
                                    if (
                                        ...
                                        try:
                                            ...
                                            logger_db_converter.debug(
                                                "Convert init_env error: %s",
                                                e,
                                                exc_info=True,
                                            )
                            except Exception as e:
                                logger_db_converter.debug(
                                    "Convert init_env error: %s", e, exc_info=True
                                )
