...
QDX提供多种[推送方式](#推送方式)，你可以在 `工具箱`->`推送注册` 中注册不同的推送工具，以便在发生特定事件（例如定时任务执行失败）时向你推送通知。
...
查看推送注册前值时, 请确保 `邮箱` 和 `密码` 中填写了 QDX的用户邮箱和密码.
...
参数名|是否必须|默认值|说明
:-: | :-: | :-: | :-:
DINGDING_TOKEN|是|""|钉钉推送 Token, <br>可在 [自定义机器人接入](https://open.dingtalk.com/document/robots/custom-robot-access) 中获取, <br>如果你在 `安全设置` 中设置了 `IP地址段` , <br>请将 QD 服务器的 IP 地址添加到 `IP地址段` 中, <br>否则无法接收到推送消息; <br>如果你在 `安全设置` 中设置了 `自定义关键词` , <br>请将 `QD`/`推送`/`测试` 添加到 `自定义关键词` 中, <br>否则无法接收到推送消息; <br>请勿在 `安全设置` 中开启 `加签`, QDX暂不支持钉钉加签推送.
PUSH_PIC_URL|否|""|自定义推送图片地址, <br>如果留空, 则使用环境变量 `PUSH_PIC_URL` 值
...
WXPusher 极简推送 需要在 `工具箱`->`推送注册` 中设置参数：

WXPusher 极简推送，简单好用, 只需一个 SPT(Simple Push Token) 即可收消息, 支持 Android、iOS、Harmony、Window、MacOS、Linux 等 6 大平台（<a href='https://wxpusher.zjiecode.com/download/' target='_blank' rel='noopener noreferrer'>点击下载 APP</a>）。需要在 `工具箱`->`推送注册` 中设置参数：

参数名|是否必须|默认值|说明
:-: | :-: | :-: | :-:
SPT|是|""|WXPusher 极简推送 Token(SPT), <br>可在 [WXPusher 极简推送](https://wxpusher.zjiecode.com/docs/#/?id=%e6%9e%81%e7%ae%80%e6%8e%a8%e9%80%81) 中获取; <br>支持配置多个, 用英文逗号 `,` 分隔, 最多 10 个。

::: tip 扫码获取 SPT

使用微信扫描下方二维码, 关注后即可直接获取你的 SPT, 简单快捷:

<img src="https://wxpusher.zjiecode.com/api/qrcode/RwjGLMOPTYp35zSYQr0HxbCPrV9eU0wKVBXU1D5VVtya0cQXEJWPjqBdW3gKLifS.jpg" alt="WxPusher 极简推送二维码" width="200" />
